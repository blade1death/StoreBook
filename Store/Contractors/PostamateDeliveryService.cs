...
namespace Store.Contractors
{
    public class PostamateDeliveryService : IDeliveryService
    {
        ...
        public string Title => "Доставка через библиотек в Казани и Волжске";
            ...
            if (step == 1)
            {
                if (values["city"] == "1")
                {
                    return Form.CreateNext(Name, 2, values)
                               .AddField(new SelectionField("Библиотека", "postamate", "1", postamates["1"]));
                }
                else if (values["city"] == "2")
                {
                    return Form.CreateNext(Name, 2, values)
                               .AddField(new SelectionField("Библиотека", "postamate", "4", postamates["2"]));
                }
                else
